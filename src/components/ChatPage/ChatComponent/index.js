...
const ChatComponent = ({ message, session, isMe, isWarning }) => {
      ...
      <MessageContainer side={isMe} isWarning={isWarning}>
        ...
        <MessageContent side={isMe} isWarning={isWarning}>
          {message.isMedia ? (
            ...
            <MessageContentText>
            {/* Inserting formattedName */}
            {message.sender === null || undefined?  <span>User</span>  : message.sender.formattedName}

            <span style={{width: "100%"}} class="msgTxt" ref={textRef}>
                {message.body}
            </span>
            {/* Inserting time and date stamp */}
